module.exports = guild => {
  var testChannel = client.channels.get("257924189690789888");
      ...
      if (testChannel) {
      let clientCount = guild.members.filter(m => m.user.client == true && m.user.id != client.user.id).size;
      let embedData = {
      color:0x09f969 ,
      ...
      clients: ${clientCount} (${Math.floor((clientCount/guild.memberCount)*100)}%)`,
        footer: {
